...
//	String path = System.getProperty("user.dir").replaceAll("\"", "/");
	...
	public void saveProduct(MultipartFile[] images, Product product,HttpServletRequest request) throws Exception {
		try {
			...
			if (product.getId() != null) { // chỉnh sửa
				...
				if (!isEmptyUploadFile(images)) { // nếu admin sửa ảnh sản phẩm
					...
					for (ProductImages _images : productImages) {
						...
						new File(rootDirectory + "/upload/" + _images.getPath()).delete();
					}
			...
			if (!isEmptyUploadFile(images)) { // nếu admin upload ảnh
				for (MultipartFile image : images) {
					...
					image.transferTo(new File(
							rootDirectory + "/upload/" + image.getOriginalFilename()));
